-- cook_cc.discovery.finders.cmake_compat — cmake --find-package legacy-mode strategy: cc:cmake-driver probe plus compile/link output parsing
...
-- through `cook_cc.discovery.finder.register_find_probe` during register and keeps
...
function M.main_chain(name, opts)
    ...
    if not probe_exist(name) then
        local hints = require("cook_cc.discovery.finders.cmake_compat.hints")
