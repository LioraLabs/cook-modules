-- cook_cc.discovery.finder — registers cc:find:<name> probes and returns sigil records (find / find_or_error / uses)
...
local function produce_body(name, opts, raise_on_miss)
    ...
    return string.format([[
        local h = require("cook_cc.discovery._probe_helpers")
        local NAME = %q
        local OPTS = %s
        local tried = {}
...
local function register_find_probe(name, opts, raise_on_miss)
    ...
    require("cook_cc.discovery.finders.bare_probe").ensure_probe_registered()
    require("cook_cc.discovery.finders.cmake_compat").ensure_probe_registered()
